...
let name1 = 'maeydah';
console.log(name1 !== 'john');//true if they are not equal
let number = 2;
 ...
 */
function make_shirt1(message?:string,size?:number) {
    let sizes:string[]=["small","medium","large"];
    let defSize = 2; //default size is Medium
...
*/
function describe_city(cityCountryPair:string):string {
     let citiesCountries = [
        "Lahore,Pakistan",
        "Tokyo,Japan",
        "New,USA"]
...
function  show_magicians(arrayOfNames:any[]){
...
function  store_magicians(){
    const greatMagicians = lists; //creates a shallow copy of list
...
class Cars {
    manufacture: string;
    model: string;
    constructor(manufacure: string, model: string) {
